...
class APIResponseValidationError(APIResponseError):
	...
	def __init__(
		...
		*,
		...
		if f', url: {response.request.url}' not in msg:
			msg = f'{msg}, url: {response.request.url}'
		super().__init__(message=msg, request=response.request, json_data=json_data)
		self.response = response
		self.status_code = response.status_code
...
class APIConnectionError(APIResponseError):
	def __init__(
		...
		msg = message
		if f', url: {request.url}' not in msg:
			msg = f'{msg}, url: {request.url}'
		super().__init__(msg, request, json_data=None)
